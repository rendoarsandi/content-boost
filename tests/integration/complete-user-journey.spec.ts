...
test.describe('Complete User Journey Integration Test', () => {
  ...
  test('Complete Creator Journey: Landing → Auth → Campaign Creation → Management', async ({ page }) => {
    // 1. Start from landing page
    await page.goto('http://localhost:3000'); // Landing page
    
    // Verify landing page loaded
    await expect(page.locator('h1:has-text("Tingkatkan Engagement")')).toBeVisible();
    ...
    // Create a campaign
    await page.fill('input[name="title"]', `Integration Test Campaign ${testSessionId}`);
    await page.fill('textarea[name="description"]', 'Full integration test campaign');
    ...
    if (url.includes('/campaigns/')) {
      await expect(page.locator(`h1:has-text("Integration Test Campaign ${testSessionId}")`)).toBeVisible();
    }
  });

  test('Complete Promoter Journey: Landing → Auth → Campaign Discovery → Application', async ({ page }) => {
    // 1. Start from landing page
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /campaigns|available/i })).toBeVisible();
    ...
    if (campaignCount > 0) {
      ...
      if (await motivationField.isVisible()) {
        await motivationField.fill(`Integration test application from promoter ${testSessionId}`);
        await page.click('button[type="submit"]');
  ...
  test('Admin Oversight of User Activities', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /campaign|oversight/i })).toBeVisible();
  });
  ...
  test('Data Consistency Across Apps', async ({ page }) => {
    ...
    const campaignVisible = await page.locator(`text=${campaignTitle}`).isVisible().catch(() => false);
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /campaign/i })).toBeVisible();
  });
