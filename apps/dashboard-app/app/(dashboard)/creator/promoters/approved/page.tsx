...
export default async function ApprovedPromotersPage() {
  ...
  const approvedPromoters = await getApprovedPromoters((session.user as any).id);

  // Group promoters by campaign
  const promotersByCampaign = approvedPromoters.reduce((acc, promoter) => {
  ...
  const totalViews = approvedPromoters.reduce((sum, p) => sum + p.metrics.legitimateViews, 0);
  const totalEarnings = approvedPromoters.reduce((sum, p) => sum + p.metrics.estimatedEarnings, 0);

  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Approved Promoters</h1>
          <p className="text-gray-600 mt-2">Active promoters working on your campaigns</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">Legitimate views generated</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">Estimated promoter earnings</p>
          </CardContent>
        </Card>
      </div>
      ...
      {totalApprovedPromoters === 0 ? (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No approved promoters yet</h3>
        ...
        <div className="space-y-8">
          {Object.entries(promotersByCampaign).map(([campaignId, { campaign, promoters }]) => (
            <Card key={campaignId}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {promoters.length} approved promoter{promoters.length !== 1 ? 's' : ''}
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {promoters.map((promoter) => (
                    <div key={`${promoter.promoter.id}-${campaignId}`} className="border rounded-lg p-4">
                          ...
                          <div className="grid grid-cols-1 gap-2 text-sm">
                            ...
                            <div>
                              ...
                              <span className="font-semibold text-green-600">
                                Rp {promoter.metrics.estimatedEarnings.toLocaleString()}
                              </span>
                            </div>
                          </div>
                        </div>
