...
export class InstagramAPI {
  ...
  async getUserInfo(): Promise<InstagramUserInfo> {
    ...
    const response = await this.makeRequest<InstagramUserInfo>(`/me?fields=${fields}`);
    return response;
  }
  
  async getUserMedia(limit: number = 25, after?: string): Promise<{
  ...
  async getMediaInsights(mediaId: string, metrics: string[] = [
    ...
    return await this.makeRequest(`/${mediaId}/insights?metric=${metricsParam}`);
  }
  ...
  async getBatchMediaInsights(mediaIds: string[]): Promise<Map<string, InstagramInsights[]>> {
