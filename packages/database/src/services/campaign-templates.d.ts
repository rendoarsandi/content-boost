interface CampaignTemplate {
    ...
    name: string;
    description: string;
    category: 'product_launch' | 'brand_awareness' | 'seasonal' | 'conversion' | 'engagement' | 'user_generated_content';
    template_config: {
...
export declare class CampaignTemplateService {
    ...
    createTemplate(template: Omit<CampaignTemplate, 'id' | 'created_at' | 'updated_at'>): Promise<{
        template: CampaignTemplate | null;
        error: string | null;
    }>;
    updateTemplate(templateId: string, updates: Partial<CampaignTemplate>): Promise<{
        success: boolean;
        error: string | null;
    }>;
