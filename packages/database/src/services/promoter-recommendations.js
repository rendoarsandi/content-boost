...
class PromoterRecommendationService {
    ...
    async generateRecommendations(creatorId, campaignId, limit = 20) {
        ...
        `)
            ...
            for (const promoter of promoters || []) {
                const score = await this.calculateRecommendationScore(promoter, campaignNiche, campaignBudget, creatorId);
                if (score.total > 0.3) {
                    // Minimum threshold
            ...
            if (topRecommendations.length > 0) {
                const { error: saveError } = await this.supabase
                    ...
                    .upsert(topRecommendations.map(rec => ({
                    id: rec.id,
    ...
    async calculateRecommendationScore(promoter, campaignNiche, campaignBudget, creatorId) {
        ...
        if (campaignNiche.length > 0 && promoter.niche?.length > 0) {
            const matches = promoter.niche.filter((niche) => campaignNiche.includes(niche)).length;
            nicheMatchScore =
        ...
        if (pastCampaigns && pastCampaigns.length > 0) {
            ...
            if (analytics && analytics.length > 0) {
                const avgROI = analytics.reduce((sum, a) => sum + (a.roi || 0), 0) /
                    analytics.length;
                const avgConversionRate = analytics.reduce((sum, a) => sum + (a.conversion_rate || 0), 0) /
                    analytics.length;
        ...
        tierBonus =
            tierMultipliers[promoter.tier] || 0;
        // Weight the factors
        const weights = {
            niche_match: 0.3,
        ...
        const total = factors.niche_match * weights.niche_match +
