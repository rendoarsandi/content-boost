...
describe('Disaster Recovery Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await pgPool.query('DELETE FROM campaigns WHERE id = $1', [
  ...
  describe('PostgreSQL Disaster Recovery', () => {
    test('Should be able to backup and restore database', async () => {
      ...
      try {
        ...
        console.log(
          'pg_dump error (test may be running in environment without pg_dump):',
          error
        );
      ...
      // Verify data is gone
      const checkResult1 = await pgPool.query(
      ...
      try {
        ...
        console.log(
          'psql restore error (test may be running in environment without psql):',
          error
        );
      ...
      const checkResult2 = await pgPool.query(
        'SELECT * FROM users WHERE id = $1',
        [TEST_DATA.userId]
      );
      ...
      const resilientDbService = {
        async getUserById(userId: string) {
          try {
            ...
            console.log(
              'Database error, falling back to cache:',
              dbError.message
            );
