...
export const NotificationSchema = z.object({
  type: z.enum(['application_submitted', 'application_approved', 'application_rejected', 'materials_accessed']),
...
export class ApplicationService {
   ...
   */
  static validateTrackingLink(trackingLink: string): { valid: boolean; error?: string } {
    try {
    ...
    for (const requirement of campaignRequirements) {
      ...
      if (req.includes('followers')) {
        ...
        if (match) {
          ...
          if (!promoterProfile.followerCount || promoterProfile.followerCount < requiredFollowers) {
            missingRequirements.push(`Minimum ${requiredFollowers} followers required`);
          }
        }
      }
      ...
      if (req.includes('engagement')) {
        ...
        if (match) {
          ...
          if (!promoterProfile.engagementRate || promoterProfile.engagementRate < requiredEngagement) {
            missingRequirements.push(`Minimum ${requiredEngagement}% engagement rate required`);
          }
        }
      }
    }
   ...
   */
  static calculateApplicationMetrics(applications: Array<{
    status: string;
    appliedAt: Date;
  }>): {
    total: number;
  ...
  static validateProposedContent(
    ...
    const platformReqs = campaignRequirements.filter(req => 
      req.toLowerCase().includes('tiktok') || req.toLowerCase().includes('instagram')
    );
    
    for (const req of platformReqs) {
      if (req.toLowerCase().includes('tiktok') && proposedContent.platform !== 'tiktok') {
        issues.push('Campaign requires TikTok content');
      }
      if (req.toLowerCase().includes('instagram') && proposedContent.platform !== 'instagram') {
        issues.push('Campaign requires Instagram content');
      }
    }

    // Check content type requirements
    const contentTypeReqs = campaignRequirements.filter(req => 
    ...
    for (const req of contentTypeReqs) {
      ...
      if (reqLower.includes('video') && proposedContent.contentType !== 'video') {
    ...
    const hashtagReqs = campaignRequirements.filter(req => 
      req.toLowerCase().includes('hashtag') || req.toLowerCase().includes('#')
    );
    
    if (hashtagReqs.length > 0 && (!proposedContent.hashtags || proposedContent.hashtags.length === 0)) {
      issues.push('Campaign requires specific hashtags');
    }

    // Check minimum reach requirements
    const reachReqs = campaignRequirements.filter(req => 
      req.toLowerCase().includes('reach') || req.toLowerCase().includes('views')
    );
    ...
    if (proposedContent.description.length < 20) {
      issues.push('Content description should be more detailed (minimum 20 characters)');
    }
  ...
  static generateComprehensiveNotification(
    type: 'application_submitted' | 'application_approved' | 'application_rejected' | 'materials_accessed',
    data: {
      campaignTitle: string;
      promoterName?: string;
      creatorName?: string;
  ...
  static calculateApplicationScore(application: {
    ...
    const matchPercentage = application.campaignRequirements.length > 0 
      ? ((application.campaignRequirements.length - requirementValidation.missingRequirements.length) / application.campaignRequirements.length) * 100
      : 100;
    
    breakdown.requirementMatch = Math.round((matchPercentage / 100) * 25);
    
    if (requirementValidation.missingRequirements.length > 0) {
      recommendations.push(...requirementValidation.missingRequirements.map(req => `Address requirement: ${req}`));
    }
    ...
    if (profile.previousCampaigns) {
      ...
      recommendations.push('Complete more campaigns to build your track record');
    }
    ...
    const totalScore = breakdown.contentQuality + breakdown.profileStrength + breakdown.requirementMatch + breakdown.experience;
...
export class ApplicationNotFoundError extends ApplicationError {
  constructor(applicationId: string) {
    super(`Application with ID ${applicationId} not found`, 'APPLICATION_NOT_FOUND', 404);
  }
}
...
export class CampaignNotActiveError extends ApplicationError {
  constructor() {
    super('Campaign is not currently active or accepting applications', 'CAMPAIGN_NOT_ACTIVE', 400);
  }
}

export class DuplicateApplicationError extends ApplicationError {
  constructor() {
    super('You have already applied to this campaign', 'DUPLICATE_APPLICATION', 400);
  }
}